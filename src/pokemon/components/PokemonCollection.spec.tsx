...
jest.mock("../hooks/use-fetch-pokemons.hook");
jest.mock("../hooks/use-fetch-pokemon-info.hook");

const mockedUseFetchPokemons = useFetchPokemons as jest.Mock<any>;
const mockedUseFetchPokemonInfo = useFetchPokemonInfo as jest.Mock<any>;

describe("PokemonCollection", () => {
  beforeEach(() => {
    mockedUseFetchPokemons.mockImplementation(() => ({ isLoading: true }));
    mockedUseFetchPokemonInfo.mockImplementation(() => ({ isLoading: true }));
  });
  ...
  it("should display the loading indicator", () => {
    const { getByRole } = render(<PokemonCollection />);

    expect(getByRole("progressbar")).toHaveTextContent("Loading...");
  });
  ...
  it("should display data", () => {
    const pokemons = {
      ...
      data: {
        ...
        previous: null,
        next: null,
        results: [{ name: "ditto", url: "https://test.url/testpokemon" }],
      },
    };

    const generatePokemonInfo = (name: string): PokemonInfo => ({
      abilities: [
        {
          ability: {
            name: "limber",
    ...
    mockedUseFetchPokemons.mockImplementation(() => pokemons);
    mockedUseFetchPokemonInfo.mockImplementation((name) => ({
      isLoading: false,
      data: generatePokemonInfo(name),
    }));

    const { getByRole, getAllByRole } = render(<PokemonCollection />);
    expect(getByRole("heading")).toHaveTextContent("Pokedex");
    expect(getByRole("list")).toBeInTheDocument();
    expect(getAllByRole("listitem")).toHaveLength(pokemons.data.count);
  });

  it("should display the error message", () => {
    const error = "Error message";
    mockedUseFetchPokemons.mockImplementation(() => ({
      isLoading: false,
      error,
    }));

    const { getByText } = render(<PokemonCollection />);

    expect(
      getByText(`An error has occurred: ${JSON.stringify(error)}`)
    ).toBeInTheDocument();
  });
});
